...
!
! gfortran rate06_reduce_20120328.o common.o analyse_reduce.o subroutines_share_triv.o opkd*.o
! Small syntax modification to adapt to the gfortran in my Mac laptop.
! <timestamp>2013-05-28 Tue 14:24:37</timestamp>
...
! Initialize variables of ODE solver.
      ...
      allocate (y(NEQ), ytmp(NEQ), RWORK(LRW), IWORK(LIW), STAT=statALLOC)
...
!-- Beginning of the big loop.
      ...
      do iPhyPar=1, nPhyPar
        ...
        n_H = n_H_s(iPhyPar)
        Av = Av_s(iPhyPar)
        write (*,'(/I2,"/",I3, " T = ", F10.1, " n = ", ES10.1, " Av = ", F10.1)') &
          iPhyPar, nPhyPar, Temperature, n_H, Av
        ...
        !write(*,*) 'Initial elemental composition: '
        !do i=1, nElement
        !  write(*,*) nameElements(i), dot_product(y, SpeciesElements(i, :))
        !end do
        !
        CALL SYSTEM_CLOCK (ClockCount0, CountRate)
        do i=1, nIteration
          ...
          !if (maxval(abs(yHistory(:,i)-y) / (yHistory(:,i)+y+1D-100)) &
          !  .LE. 1D-200) then
          !  do j=i+2, nHistoryLen
          !    yHistory(:, j) = y
          !    touts(j) = t
          !  end do
          !  exit
          !end if
          !write(*,'(ES14.4, A, ES14.4/)') t, 'C', dot_product(y, SpeciesElements(7, :))
        end do
        ...
        do i=1, nIteration
          ...
          !if (i .GT. 100) then
          !  if (maxval(abs(yHistory(:,i-2)-y) / (yHistory(:,i-2)+y+1D-50)) &
          !    .LE. 1D-200) then
          !    do j=i+2, nHistoryLen
          !      yHistory(:, j) = y
          !      touts(j) = t
          !    end do
          !    touts(i+2:nHistoryLen) = t
          !    exit
          !  end if
          !end if
        end do
        !
        write(*,*) 'CMP elemental composition: '
        do i=1, nElement
          ytmp = y_final_cmp * SpeciesElements(i, :)
          i1 = maxloc(abs(ytmp), 1)
          write(*,*) nameElements(i), dot_product(y_final_cmp, SpeciesElements(i, :)),  ' Max ', nameSpecies(i1), y_final_cmp(i1)
        end do
        !
        write(*,*) 'Final elemental composition: '
        do i=1, nElement
          ytmp = y * SpeciesElements(i, :)
          i1 = maxloc(abs(ytmp), 1)
          write(*,*) nameElements(i), dot_product(y, SpeciesElements(i, :)),  ' Max ', nameSpecies(i1), y(i1)
        end do
        !
        CALL SYSTEM_CLOCK (ClockCount1, CountRate)
        write (*, '(A, F10.3)') 'Seconds elapsed: ', &
        ...
        do i=1, nSpecies
          cmp_ratios(i) = max(y(i), y_final_cmp(i)) / (min(y(i), y_final_cmp(i)) + 1D-100)
        end do
        ...
        !end where
        !where ((yHistory(:, nHistoryLen) .LE. abundMaxHeavy) .AND. &
        !       ((y_final_cmp+yHistory(:, nHistoryLen)) .LE. abundMaxHeavy))
        !  cmp_ratios = 1D0
        !end where
        ! exp(abs(log(yHistory(:, nHistoryLen) / (y_final_cmp + 1D-100))))
        write (*, '("Max ratio among all: ", ES12.2, 2X, A12)') &
          maxval(cmp_ratios), nameSpecies(maxloc(cmp_ratios, 1))
        i1 = maxloc(cmp_ratios, 1, flag_care_spe)
        write (*, '("Max ratio among those you care:", ES10.2, 2X, A12, 2ES12.3)') &
          maxval(cmp_ratios, 1, flag_care_spe .AND. &
                                ((y_final_cmp+y .GT. abundMaxHeavy))), &
          nameSpecies(i1), y_final_cmp(i1), y(i1)
        write (*, '("Num of care species with ratio >2:", I4)') &
          count(flag_care_spe .AND. (cmp_ratios .GT. 2.0) &
            .AND. ((y_final_cmp+y .GT. abundMaxHeavy)))
        write (*, '("Num of care species with ratio >5:", I4)') &
          count(flag_care_spe .AND. (cmp_ratios .GT. 5.0) &
            .AND. ((y_final_cmp+y .GT. abundMaxHeavy)))
        write (*, '("Num of care species with ratio >10:", I4)') &
        ...
        do i=1, nSpecies
          ...
          if (flag_care_spe(i)) then
            ...
            str_format = '(A12, 2ES14.2E4, 2(2X, F11.4), 3X,"*")'
            else
            str_format = '(A12, 2ES14.2E4, 2(3X, F9.2), 3X,"!")'
            end if
          else
            str_format = '(A12, 2ES14.2E4, 2(3X, F9.2))'
          end if
...
999   write (*,'(A)') &
      ...
      do i=1, nReactions
        ...
        do j=1, nRealReactants(i)
          ydot(reactions(j, i)) = ydot(reactions(j, i)) - rtmp
        end do
      ...
      do i=1, nReactions
        ...
        select case (iType(i))
          case (5)
            if (j .EQ. reactions(1, i)) then
              if (reactions(1,i) .ne. reactions(2,i)) then
                rtmp = rates(i) * y(reactions(2, i))
              else
                rtmp = 2D0*rates(i) * y(reactions(2, i))
              end if
            else if (j .EQ. reactions(2, i)) then
              if (reactions(1,i) .ne. reactions(2,i)) then
                rtmp = rates(i) * y(reactions(1, i))
              else
                rtmp = 2D0 * rates(i) * y(reactions(1, i))
              end if
            else
              rtmp = 0D0
            end if
        ...
        do k=1, nRealReactants(i)
          pdj(reactions(k, i)) = pdj(reactions(k, i)) - rtmp
        end do
        do k=nReactants+1, nReactants+nRealProducts(i)
          pdj(reactions(k, i)) = pdj(reactions(k, i)) + rtmp
        end do
      end do
      end subroutine jac
      ...
      namelist /ReduceParameters/ file_imp_species_in, &
        nPhyPar, Temperature_s, n_H_s, Av_s, &
